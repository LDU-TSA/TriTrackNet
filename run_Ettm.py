...
if __name__ == '__main__':
    # read data
    (x_train, y_train), (x_val, y_val), (x_test, y_test) = read_ETTm1_dataset(seq_len=512, pred_len=336)

    # train model
    model = TriTrackNet(device='cuda', num_epochs=20, batch_size=8, base_optimizer=torch.optim.AdamW,
                      learning_rate=1e-3, weight_decay=1e-4, rho=0.1, use_revin=True,steps=3,epsilon=1.5)

    model.fit(x_train, y_train)

    # eval results
    y_pred_test_mu, y_pred_test_sigma = model.predict(x_test)

    # Calculate RMSE, MSE, and MAE
    rmse = np.sqrt(np.mean((y_test - y_pred_test_mu) ** 2))
    mse = np.mean((y_test - y_pred_test_mu) ** 2)
    mae = np.mean(np.abs(y_test - y_pred_test_mu))
    ...
    # 你可以选择使用 sigma 进行额外的分析，例如通过标准差来评估不确定性：
    uncertainty = np.mean(y_pred_test_sigma)  # 计算预测的不确定性（方差）
    print('Uncertainty (mean sigma):', uncertainty)
