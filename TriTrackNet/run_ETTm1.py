...
def read_ETTm1_dataset(seq_len, pred_len, time_increment=1):
    file_name = 'E:\\TriTrackNet\\dataset\\ETTm2.csv'
...
def plot_prediction_comparison(y_true, y_pred, title="Prediction Comparison"):
    """
    可视化预测与实际值的对比
    """
    ...
    # 绘制真实值和预测值的对比
    plt.plot(np.arange(len(y_true[0])), y_true[0], label="Ground Truth", color='blue')  # 真实值
    plt.plot(np.arange(len(y_pred[0])), y_pred[0], label="Forecast", color='orange')  # 预测值
...
if __name__ == '__main__':
    # 读取数据
    (x_train, y_train), (x_val, y_val), (x_test, y_test) = read_ETTm1_dataset(seq_len=512, pred_len=96)

    # 创建并训练模型
    model = TriTrackNet(device='cuda', num_epochs=30, batch_size=256,
    ...
    # 可视化预测结果
    plot_prediction_comparison(y_test, y_pred_test)
